...
def ask():
    ...
    x = back.get_width()
    y = back.get_height()
    resolution.set(' 图片分辨率：'+str(x)+'x'+str(y))
    path.set('图片地址：'+pic_path)
...
def save():
    ...
    if pic_path == '':
        ...
        return
    if os.path.exists(os.getcwd()+'\mosaic.jpg'):
        enter = msgbox.askokcancel('确认','mosaic.jpg已存在,要覆盖它吗?')
    if not enter:
        return
    pygame.image.save(back2,'mosaic.jpg')
    #a = filedialog.asksaveasfilename(initialdir = os.getcwd(),filetypes = [('图片','.jpg')])
...
draw = tk.Button(em,text = '开始',width = 6,height = 1,command = game)
