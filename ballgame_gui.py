...
def game():
    ...
    pygame.draw.line(screen, GOLD, (int(cursorx - plat_length / 2), height), (int(cursorx + plat_length / 2), height),
                     spline_width)

    if lose:
        screen.fill(0)
        fontrect = fonts.render_to(screen, (int(width / 2 - 120), int(height / 2 - 50)), "你输了！", fgcolor=GOLD,
                                   size=50)
        fontrect2 = fonts.render_to(screen, (int(width / 2 - 120), int(height / 2 - 100)), f"成绩：{str(score)}",
                                    fgcolor=GOLD, size=30)
...
difficulty_scale = tk.Scale(root, from_=1, to=4, resolution=1, orient=tk.HORIZONTAL, command=change, label='难度',
                            length=200)
